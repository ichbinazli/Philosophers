...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2024/07/01 14:50:15 by naanapa           #+#    #+#             */
/*   Updated: 2024/07/28 17:22:41 by naanapa          ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	init_mutex(t_rules *rules)
{
	...
	if (!destroy_mutex(rules))
		return (free(rules->philosophers), free(rules->forks), 1);
	while (--i >= 0)
	{
...
int	destroy_mutex(t_rules *rules)
{
	...
	else if (pthread_mutex_init(&(rules->died_check), NULL))
